...
class LevelFaceData:
    def __init__(self, face_count: int):
        ...
        self._is_reshaped = False
        self.origins = np.zeros(face_count * 3, dtype=np.float32)
        self.texture_us = np.zeros(face_count * 3, dtype=np.float32)
        self.texture_vs = np.zeros(face_count * 3, dtype=np.float32)
        self.poly_flags = np.zeros(face_count, dtype=np.int32)
        self.material_indices = np.zeros(face_count, dtype=np.int32)
        self.dirty_flags = np.zeros(face_count, dtype=np.bool_)
...
def apply_level_to_brush_mapping(level_object: Object) -> BrushMappingResult:
    ...
    for brush in level_object.bdk.level.brushes:
        # Note here that the brush list is the list of non-instanced brushes, so we cannot use `enumerate` and must
        # instead use the recorded index of the brush in the list.
        ...
        level_to_brush_material_index_mapping = {}

        # Get a set of the level material indices that are used by the polygons in this brush.
        brush_level_material_indices = set(map(lambda i: level_face_data.material_indices[i], mapping[brush_index].values()))
        ...
        for level_material_index in brush_level_material_indices:
            ...
            level_to_brush_material_index_mapping[level_material_index] = brush_material_slot_index

        for brush_polygon_index, level_polygon_index in mapping[brush_index].items():

            if not level_face_data.dirty_flags[level_polygon_index]:
                # Ignore non-dirty polygons.
                continue
            ...
            origin = Vector(level_face_data.origins[level_polygon_index])
            texture_u = Vector(level_face_data.texture_us[level_polygon_index])
            texture_v = Vector(level_face_data.texture_vs[level_polygon_index])
            ...
            # TODO: To make this stable regardless of brush transform, store the original brush transform in the level object.
            ...
            brush_poly_flags_attribute.data[brush_polygon_index].value = level_face_data.poly_flags[level_polygon_index]
            brush_material_index_attribute.data[brush_polygon_index].value = level_to_brush_material_index_mapping[
                level_face_data.material_indices[level_polygon_index]
            ]
...
def ensure_bdk_level_visibility_node_tree(level_object: Object):
    ...
    def build_function(node_tree: NodeTree):
        ...
        # Set all default values on the sockets to True.
        for input_socket in poly_flags_group_node.inputs:
        ...
        bsp_surface_attributes_node = node_tree.nodes.new(type='GeometryNodeGroup')
