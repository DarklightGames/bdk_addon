...
class AssetCatalogFile:
    ...
    def ensure_catalog_exists(self, name, path="") -> str:
        """Ensure that a catalog exists, and if it doesn't, create one."""
        path = path or name
        if name not in self.catalogs:
            return self.add_catalog(name, path)
        else:
            return self.catalogs[name].uuid

    def get_catalog_id(self, path: str) -> Optional[str]:
        """Get the UUID of a catalog by name"""
        if path in self.catalogs:
            return self.catalogs[path].uuid
        print('Catalog not found, dumping list of catalog paths:')
        import pprint
        pprint.pprint(self.catalogs)
        return None
