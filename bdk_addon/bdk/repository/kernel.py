...
def save_repository_manifest(repository: BDK_PG_repository, manifest: Manifest):
    manifest_path = Path(repository.cache_directory) / repository.id / 'manifest.toml'
...
def repository_runtime_update(repository: BDK_PG_repository):
    ...
    runtime.up_to_date_package_count = sum(1 for package in runtime.packages if package.status == 'UP_TO_DATE')
    runtime.need_export_package_count = sum(1 for package in runtime.packages if package.status == 'NEEDS_EXPORT')
    runtime.need_build_package_count = sum(1 for package in runtime.packages if package.status == 'NEEDS_BUILD')
